...
class ProudctCreateTestCase(APITestCase):
    def test_create_product(self):
        ...
        self.assertEqual(float(response.data['current_price']), float(product_attrs['price']),)
...
class ProductUpdateTestCase(APITestCase):
    def test_update_product(self):
        ...
        response = self.client.patch('/api/v1/products/{}/'.format(product.id),{'name':'New Product','description':'Awesome product','price':123.45}, format='json')
